...
{
	long long start_sleep = get_time(data);

	print_status(data, data->philo[0].philo, "is sleeping", 0);
    while (get_time(data) - start_sleep < data->time_to_sleep) 
	{
        if (get_time(data) - time_last_meal >= data->time_to_die) 
		{
            data->philo_dead = true;
		    printf("%lld %d died\n", get_time(data) - data->start_time, data->philo[0].philo + 1);
    	    break;
        }
        usleep(100);
    }
...
{
	long long current_time = get_time(data);
